...
AzureDust-Elf 是一个本地优先的自媒体内容创作自动化系统。它用统一的 workflow 和多 agent 分工，把一个选题拆解成微信公众号、小红书、抖音、视频号、B站五个平台的内容包，并提供中文创作工作台、后端控制台、任务队列、续跑修复、素材/视频生产包和本地交付目录。

项目默认只在本机生成内容和文件，不会自动发布到任何平台。除非你显式配置外部语音服务相关环境变量，系统也不会调用外部模型。

## 主要功能

- 一个选题生成五个平台内容：公众号文章、小红书笔记、短视频脚本、字幕、封面提示词、B站简介和章节等。
- 中文创作工作台：输入选题、上传文本/图片/视频素材、查看队列状态、按平台预览和下载生成内容。
...
创作工作台用于日常内容生成：输入选题，点击 `+` 上传文本、图片或视频素材，加入生成队列，完成后点击“查看”，选择平台预览并下载内容。
